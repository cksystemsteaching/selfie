...
class ModelConfigParser:
    ...
    def __init__(self, model_type_name: str):
        ...
        self.model_type_name = model_type_name.split("-")
        self.check()

    def check(self):
        current_level = self.top_level

        for level in self.model_type_name:
            if level in current_level:
                current_level = current_level[level]
            else:
                raise ParsingError(self.__class__.__name__, self.model_type_name, level)
