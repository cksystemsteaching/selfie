...
uint64_t X86_JA      = 131;                   //ja   imm                                 0x87 (unsigned) //FIXME (131 -> JAE/ 135 -> JA)
uint64_t X86_JE      = 132;                   //je   imm                                 0x84
uint64_t X86_JG      = 143;                   //jg   imm                                 0x8f (signed)
...
uint64_t MAX_BINARY_LENGTH = 327680; // 320KB = MAX_CODE_LENGTH + MAX_DATA_LENGTH

uint64_t MAX_CODE_LENGTH = 311296; // 304KB
...
void x86Translate(){
  ...
  dataLength = binary_length - code_length;
  printf3("x86Translate: binary_length=%d, code_length=%d, dataLength=%d\n", (char*)binary_length, (char*)code_length, (char*)dataLength);
  binary_length = 0;
...
void translate_lui() {
  ...
  //  if (binary_gp_address == 0)
  //    binary_gp_address = binary_length * SIZEOFUINT64 + x86ByteCount + 2;
  //}
...
void translate_sub() {
  ...
  if (rs1 == REG_ZR) {
    ...
    *(instr_bytes + 5) = 192 + x86GetModRMValue(op3, op2);
    ...
    //*(instr_bytes + 3) = x86GetPrefix(op1, op2, 1);
    //*(instr_bytes + 4) = X86_MOV_R;
    //*(instr_bytes + 5) = 192x + x86GetModRMValue(op1, op2);
  }
  else {
    length = 6;
    op2 = x86GetRegister(rs1);
    ...
    *(instr_bytes + 5) = 192 + x86GetModRMValue(op1, op3);
    ...
    //*(instr_bytes + 6) = x86GetPrefix(op1, op3, 1);
    //*(instr_bytes + 7) = X86_MOV_R;
    //*(instr_bytes + 8) = 192 + x86GetModRMValue(op1, op3);
  }
...
void translate_sltu() {
  ...
  if (rd != REG_ZR) {
    ...
    if (rs1 == REG_ZR) {
      ...
      //*(instr_bytes + 0) = x86GetPrefix(op2, 0, 1); 
      //*(instr_bytes + 1) = X86_MOVI + right_shift(left_shift(op2,61),61);
      //*(instr_bytes + 2) = right_shift(left_shift(0, 56), 56);
      //*(instr_bytes + 3) = right_shift(left_shift(0, 48), 56); 
      //*(instr_bytes + 4) = right_shift(left_shift(0, 40), 56);
      //*(instr_bytes + 5) = right_shift(left_shift(0, 32), 56);
      //*(instr_bytes + 6) = right_shift(left_shift(0, 24), 56); 
      //*(instr_bytes + 7) = right_shift(left_shift(0, 16), 56); 
      //*(instr_bytes + 8) = right_shift(left_shift(0, 8),  56);
      //*(instr_bytes + 9) = right_shift(0, 56);
      ...
      *(instr_bytes + 2) = 192 + x86GetModRMValue(op2, op3);
      ...
      //*(instr_bytes + 3) = right_shift(left_shift(0, 56), 56);
      //*(instr_bytes + 4) = right_shift(left_shift(0, 48), 56);
      //*(instr_bytes + 5) = right_shift(left_shift(0, 40), 56);
      //*(instr_bytes + 6) = right_shift(left_shift(0, 32), 56);
    }
    else {
      length = 3;
      op2 = x86GetRegister(rs1);
...
void translate_ld() {
  ...
  if (imm == 0) {
    if (rs1 == REG_SP) {
      ...
      //if (right_shift(left_shift(ir, 44), 44) == 65811) { // 65811 -> addi sp,sp,x
      if (get_opcode(ir) == OP_IMM) {
      decode_i_format();
	if (funct3 == F3_ADDI) {
	  if (op1 > 7) {
