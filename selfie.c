...
uint64_t  get_trace_src(uint64_t index)    { return *(saddrs + index);}
...
void init_symbolic_engine() {
  ...
  saddrs  = zalloc(MAX_TRACE_LENGTH * SIZEOFUINT64);
...
void constrain_addi() {
  ...
  if (rd != REG_ZR) {
    ...
    if (is_from_return(rs1)) {
        ...
        if (*(reg_saddr + rs1)) {
          vaddr = *(reg_saddr + rs1);
          src   = *(reg_saddr + load_symbolic_memory(pt, *(reg_saddr + rs1)));
        } else {
          vaddr = *(reg_vaddr + rs1);
          src   = *(reg_saddr + rs1);
        }
