...
void translate_to_RISC_V_assembler();
...
void emit_bootstrapping() {
  ...
  */
  ...
  // this is needed for the binary translator 
  emit_nop();
...
void selfie_output() {
  ...
  if (translate) {
    printf5("%s: %d bytes with %d instructions and %d bytes of data written into %s\n", selfie_name,
	    ...
	    (char*) x86TotalEmittedInstructions,
	    (char*) x86DataLength,
	    binary_name);
  }
  else {
...
void print_usage() {
  printf3("%s: usage: selfie { %s } [ %s ]\n", selfie_name,
    "-c { source } | -o binary | [ -s | -S | -x86 ] assembly | -l binary | -sat dimacs",
...
uint64_t selfie() {
  ...
  if (number_of_remaining_arguments() == 0)
    ...
    while (number_of_remaining_arguments() > 0) {
      ...
      else if (string_compare(option, "-x86"))
	selfie_translate();
