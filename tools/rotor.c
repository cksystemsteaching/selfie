...
void open_model_file() {
  ...
  if (printing_unrolled_model > 0)
    w = w + dprintf(output_fd, "; unrolled %s for %lu steps\n\n", model_name, printing_unrolled_model);
...
void print_unrolled_model() {
  ...
  while (1) {
    ...
    if (next_step - current_offset >= printing_unrolled_model) {
      ...
      printf("%s: unrolled %s for %lu steps\n", selfie_name,
        model_name, next_step - current_offset);
  ...
  while (1) {
    if (number_of_remaining_arguments() > 1) {
      if (string_compare(peek_argument(1), evaluate_model_option)) {
        ...
        if (number_of_remaining_arguments() > 1) {
          printing_unrolled_model = atoi(peek_argument(1));
