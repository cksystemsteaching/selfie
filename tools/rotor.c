...
void open_model_file() {
  ...
  w = w + dprintf(output_fd, " file %s generated by %s\n\n", model_name, selfie_name);
  if (printing_unrolled_model > 0)
    w = w + dprintf(output_fd, "; unrolled for %lu steps (up to %lu instructions)\n\n",
      printing_unrolled_model - 1, printing_unrolled_model);
