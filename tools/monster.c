...
// +----+-----------------+
// | 24 | execution depth | number of executed instructions
// | 25 | path condition  | pointer to path condition
// | 26 | symbolic memory | pointer to symbolic memory
// | 27 | symbolic regs   | pointer to symbolic registers
// | 28 | beq counter     | number of executed symbolic beq instructions
// | 29 | merge location  | program location at which the context can possibly be merged (later)
// | 30 | merge partner   | pointer to the context from which this context was created
// | 31 | call stack      | pointer to a list containing the addresses of the procedures on the call stack
// +----+-----------------+

uint64_t* allocate_symbolic_context() {
  return smalloc(9 * SIZEOFUINT64STAR + 15 * SIZEOFUINT64 + 5 * SIZEOFUINT64STAR + 3 * SIZEOFUINT64);
}

uint64_t  get_execution_depth(uint64_t* context) { return             *(context + 24); }
char*     get_path_condition(uint64_t* context)  { return (char*)     *(context + 25); }
uint64_t* get_symbolic_memory(uint64_t* context) { return (uint64_t*) *(context + 26); }
uint64_t* get_symbolic_regs(uint64_t* context)   { return (uint64_t*) *(context + 27); }
uint64_t  get_beq_counter(uint64_t* context)     { return             *(context + 28); }
uint64_t  get_merge_location(uint64_t* context)  { return             *(context + 29); }
...
void set_execution_depth(uint64_t* context, uint64_t depth)   { *(context + 24) =            depth; }
void set_path_condition(uint64_t* context, char* path)        { *(context + 25) = (uint64_t) path; }
void set_symbolic_memory(uint64_t* context, uint64_t* memory) { *(context + 26) = (uint64_t) memory; }
void set_symbolic_regs(uint64_t* context, uint64_t* regs)     { *(context + 27) = (uint64_t) regs; }
void set_beq_counter(uint64_t* context, uint64_t counter)     { *(context + 28) =            counter; }
void set_merge_location(uint64_t* context, uint64_t location) { *(context + 29) =            location; }
