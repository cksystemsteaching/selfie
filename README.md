...
Translating 32-bit virtual to physical addresses involves replacing the 20 MSBs of a 32-bit virtual address with the 20-bit number of the frame to which the page is mapped that is identified by the 20 MSBs of the 32-bit virtual address. The procedure `tlb` in selfie does just that! The name `tlb` is short for *translation lookaside buffer* which is in reality an *associative cache* for speeding up the process of finding the frame number of a given page number. However, the procedure `tlb` does not do any caching to keep things simple, but real hardware does.
...
Well, but there are a million pages in just one virtual address space. Mapping all of them requires a data structure commonly called a *page table* that can hold up to a million frame numbers. The actual number of bits needed to identify each frame depends on the size of physical memory available for paging. However, in selfie, we simply dedicate a full machine word of 64 or 32 bits to each frame number, depending on whether selfie runs on a 64-bit or 32-bit system, respectively. So, how many bytes are needed for a page table that can map a million pages? Easy. There are `2^20` 4KB-pages, each mapped to a 64-bit or 8-byte  frame number on a 64-bit system, requiring `2^23` or 8MB bytes in total since 8 bytes is `2^3` bytes. On a 32-bit system, it is 4MB in total. Not so bad, but still quite a bit.
