...
Virtual memory in most modern systems, and even in selfie, is implemented with *paging*. The idea is to partition the address space of virtual memory into *pages* of the same size. The most common size in modern systems is 4KB per page which selfie adopts as well. Other, usually much bigger page sizes are also supported by some systems to reduce spatial overhead for managing pages but not in selfie. Let us do a few simple calculations. How many 4KB-pages exist in a 4GB-address space? In powers of two, the calculation is easy to do even without a calculator. There are `2^32` bytes in a 4GB-address space and `2^12` bytes in a 4KB-page. Dividing `2^32` by `2^12` amounts to subtracting 12 from 32, that is, there are `2^20` 4KB-pages in a 4GB-address space. In decimal notation, this is 1048576 pages or roughly a million pages. Thus we need 20 bits to be able to identify each page. In fact, the 20 most significant bits in a 32-bit virtual address do just that. We call them *page number*. The remaining 12 least significant bits identify each byte in a given page.
