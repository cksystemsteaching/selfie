...
Let us take a look at the code shown in the above figure. It first checks if the value of `character` is a letter. In that case, the code would be looking for an identifier. We deal with that case below. If not, the code checks if the value of `character` is a digit. Say, per our example the value of `character` is equal to `8` as indicated by the green line, so yes. The `if` statement, or rather the `if` condition evaluating to true, takes us to the red start state of our FSM as indicated by the blue line. The following `if` statement checks if the value of `character` is equal to `'0'` or not. If yes, we are done. This check implements the `|` operator in the EBNF production `integer = "0" | non_zero_digit { digit } .`.

Computing the numerical value of `'0'` is trivial in this case. It is just `0`. Notice the difference between `'0'` and `0`. The former is the ASCII code of `0` which happens to be the decimal value `48`, not the decimal value `0`. This is our first and most basic example of the difference of syntax and semantics! The character `'0'` is syntax, the value `0` is semantics, that is, the value `0` is what the character `'0'` actually means.

> Preconditions, postconditions, and invariants!

The other important item here is the call to `get_character()` which reads the next character and stores it in `character`, to prepare for subsequent calls to the scanner. Thereby, the scanner maintains something very important, a property known as *invariant*: `character` always contains the character that is to be scanned next. In other words, upon calling the scanner, we can assume the invariant holds as a *precondition*. This means that upon calling the scanner for the first time, we actually have to call `get_character()` once before that. Try finding that call in the selfie code!

Then, the scanner scans characters and, when done, needs to make sure that the invariant holds again as a *postcondition* by calling `get_character()` once more before terminating to prepare for the next call to the scanner. Notice that the invariant is temporarily violated whenever the scanner begins examining `character`, making it the currently scanned character rather than the next character to scan, until it calls `get_character()` again, reestablishing the invariant. This may look like nitpicking but reasoning in terms of invariants as well as pre- and postconditions helps tremendeously when developing code in imperative programming languages.

Every time you would like to write down a statement in, say, C\* think of which property of the program state needs to hold before executing the statement so that executing the statement works as intended. This is the precondition. And then think of which property of the program state holds after executing the statement, assuming the precondition is true. This is the postcondition which becomes the precondition for the next statement and so on. Invariants apply to iterative and recursive code, that is, to loops, as we see next, and recursive procedures, which here are our parser procedures that keep calling the scanner, as we see further below. In short, an invariant is a property that is both pre- and postcondition of a loop or recursive procedure. When reasoning about loops and recursive procedures, the key question is what exactly the invariants of those loops and recursive procedures are, which is usually harder to do than just figuring out pre- and postconditions of sequential code.

Let us go back to the `if` statement that checks if the value of `character` is equal to `'0'`. In our example, the value of `character` is equal to `'8'`, so no. This takes us to the `else` case. The first statement allocates memory on the heap to accommodate the largest unsigned integer literals we can handle which requires 20+1 bytes for a string of 20 decimal digits and its null-termination. Recall that the largest unsigned integer value in selfie is 2^64^-1 which is 18446744073709551615 in decimal, a number with 20 digits. Interestingly, that limitation neither shows up in the regular expression nor the FSM. Both impose no bound on the length of accepted sequences of characters. Well, we could implement support of integer literals whose length in decimal notation would only be bounded by the available memory of the machine. In fact, there are programming languages and in particular libraries that do that but you guessed right, it is unnecessarily complex to do that here. So, the reality is that an implementation, ours included, is often just an approximation of an ideal specification and model.

> `malloc()`: dynamic memory allocation on the heap

The procedure `string_alloc()` allocates memory using the infamous procedure `malloc()` which stands for *memory allocate* on the heap. That procedure is available in virtually all dialects of C including C\* and so important that it is even built into the language. C compilers including the selfie compiler add code that implements the procedure to any compiled code that uses it. Programming languages other than C likely feature builtin procedures similar to `malloc()` such as Java, for example, where its counterpart is called `new()`.
