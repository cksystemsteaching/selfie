...
Selfie is implemented in a single (!) file and kept minimal for simplicity. There is also a simple in-memory linker, a RISC-U disassembler, a conservative garbage collector, a profiler, and a debugger with replay as well as minimal operating system support built into the emulator. The garbage collector may operate as library in the same address space as the mutator and as part of the emulator in the address space of the kernel.
