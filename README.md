...
Let us take a look at the code shown in the above figure. It first checks if `character` is a letter. In that case, the code would be looking for an identifier. We deal with that case below. If not, the code checks if `character` is a digit. Say, per our example `character` is equal to `8` as indicated by the green line, so yes. The `if` statement, or rather the `if` condition evaluating to true, takes us to the red start state of our FSM as indicated by the blue line. The following `if` statement checks if `character` is equal to `'0'` or not. If yes, we are done. This check implements the `|` operator in the EBNF production `integer = "0" | non_zero_digit { digit } .`.
...
Let us go back to the `if` statement that checks if `character` is equal to `'0'`. In our example, `character` is equal to `'8'`, so no. This takes us to the `else` case. The first statement allocates memory on the heap to accommodate the largest unsigned integer literals we can handle which requires 20+1 bytes for a string of 20 decimal digits and its null-termination. Recall that the largest unsigned integer value in selfie is 2^64^-1 which is 18446744073709551615 in decimal. Interestingly, that limitation neither shows up in the regular expression nor the FSM. Both impose no bound on the length of accepted sequences of characters. Well, we could implement support of integer literals whose length in decimal notation would only be bounded by the available memory of the machine. In fact, there are programming languages and in particular libraries that do that but you guessed right, it is unnecessarily complex to do that here. So, the reality is that an implementation, ours included, is often just an approximation of an ideal specification and model.
