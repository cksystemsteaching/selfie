...
> Paging pages to page frames

Virtual memory in most modern systems, and even in selfie, is implemented with *paging*. The idea is to partition the address space of virtual memory and physical memory into same-size blocks called *pages* and *page frames*, respectively, and then map or *page* individual pages to page frames. The most common size in modern systems is 4KB per page and page frame which selfie adopts as well. Other, usually much bigger page sizes are also supported by some systems to reduce the overhead of managing pages and page frames but not in selfie. Either way, larger page sizes increase the chance of internal fragmentation.

Let us do a few simple calculations. How many 4KB-pages exist in a 4GB-address space? In powers of two, the calculation is easy to do even without a calculator. There are `2^32` bytes in a 4GB-address space and `2^12` bytes in a 4KB-page. Dividing `2^32` by `2^12` amounts to subtracting 12 from 32, that is, there are `2^20` 4KB-pages in a 4GB-address space. In decimal notation, this is 1048576 pages or roughly a million pages. Thus we need 20 bits to be able to identify each page. In fact, the 20 most significant bits in a 32-bit virtual address do just that. We call them the *page number*. The remaining 12 least significant bits identify each byte within a given page. The same applies to 32-bit physical addresses and *page frame numbers*. Depending on the size of physical memory available for paging, more or less than 20 bits may be needed to identify every page frame. However, for simplicity, we just use 20 bits as well. Thus mapping pages to page frames requires mapping 20-bit page numbers to 20-bit page frame numbers.

> Address translation

Translating 32-bit virtual to physical addresses involves replacing the 20 MSBs of a virtual address with the 20-bit number of the page frame to which the page is mapped that is identified by the 20 MSBs of the virtual address. The procedure `tlb` in selfie does just that! The name `tlb` is short for *translation lookaside buffer* which is an *associative cache* for speeding up the process of finding the page frame number of a given page number. However, the procedure `tlb` does not do any caching to keep things simple, but real hardware does.

> Pages are virtual addresses, page frames are physical storage

Paging allows mapping individual pages to page frames anywhere in physical memory. The order in which page frames appear in physical memory is irrelevant. Moreover, there is no external fragmentation among page frames in physical memory since they are all the same size. Thus any choice of page frames is as good as any other. In selfie, the procedure that allocates page frames is called `palloc` which implements a simple bump pointer allocator over the available physical memory. There is also a procedure called `pfree` which we nevertheless left empty to have room for future exercises. So, for now allocated page frames are never deallocated in selfie.

> Page table

Well, but there are a million pages in just one virtual address space. Mapping all of them requires a data structure commonly called a *page table* that can hold up to a million page frame numbers. The actual number of bits needed to identify each page frame depends on the size of physical memory available for paging. However, in selfie, we simply dedicate a full machine word of 64 or 32 bits to each page frame number, depending on whether selfie runs on a 64-bit or 32-bit system, respectively. So, how many bytes are needed for a page table that can map a million pages? Easy. There are `2^20` 4KB-pages, each mapped to a 64-bit or 8-byte page frame number on a 64-bit system, requiring `2^23` or 8MB bytes in total since 8 bytes is `2^3` bytes. On a 32-bit system, it is 4MB in total. Not so bad, but still quite a bit.

> On-demand paging

The most naive implementation of a page table is an array of page frame numbers indexed by page number and stored in a contiguous block of physical memory. Selfie features such an implementation with an 8MB or 4MB block depending on machine word size. The advantage of an array-based implementation is constant-time page table access and thus constant-time virtual address translation. However, recall the difference between address space and memory storage. Virtual addresses only need to be mapped upon memory access. Moreover, many programs may never access most of virtual memory, certainly not unallocated virtual memory but even not some allocated virtual memory. If virtual addresses are only mapped upon memory access, we speak of *on-demand paging*. Selfie implements on-demand paging for heap and non-initial stack segments, that is, the highest pages in virtual memory that contain the initial state of a stack. For simplicity, code, data, and initial stack segments are mapped upon loading code, data, and console arguments into virtual memory prior to executing any code. That mapping and loading is done by the procedure `boot_loader` in selfie.

> Page fault

Modern systems may implement on-demand paging for all segments of virtual memory. This is incredible as creating virtual machine instances is near instantaneous as no memory storage is allocated, not even for page tables. Only when code starts executing paging happens on demand, literally upon fetching the first instruction from virtual memory. Since there is no mapping yet, a *page fault* occurs which is a type of *exception* the CPU responds to by executing systems code, called an *exception handler*, that attempts to create a proper mapping, at least for the page where the fault occurred, and then has the CPU try again to fetch the instruction. In case of page faults in pages that contain code or data, the content is loaded into the page frame to which the faulting page is mapped. All this happens transparently to the code executing on a virtual machine. While selfie does not page code, data, and initial stack segments on demand, selfie still handles page faults on demand that happen in heap and non-initial stack segments. This is simpler because the involved page frames only need to be zeroed. In selfie, page faults are handled by the procedure `page_fault_handler`.
...
> Page tables based on trees

On-demand paging has an interesting effect on page tables when running programs that only access parts of virtual memory, as many actually do in practice. In that case, page tables tend to be *sparsely* populated meaning that many page table entries remain empty. An array-based implementation of sparsely populated page tables is obviously rather wasteful in terms of memory usage. Modern systems address the problem by implementing page tables with *tree*-like data structures. Selfie does that too, in fact, as default with a two-level tree structure.
