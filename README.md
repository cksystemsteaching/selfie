...
The other important item here is the call to `get_character()` which reads the next character and stores it in `character`, to prepare for subsequent calls to the scanner. Thereby, the scanner maintains something very important, a property known as *invariant*: `character` always contains the character that is to be scanned next. In other words, upon calling the scanner, we can assume the invariant holds as a *precondition*. This means that upon calling the scanner for the first time, we actually have to call `get_character()` before that. Try finding that call in the selfie code! Then, the scanner scans characters and, when done, makes sure that the invariant holds again as a *postcondition* to prepare for the next call to the scanner. You may have noticed that the invariant is temporarily violated whenever the scanner began examining `character` until it calls `get_character()` again, reestablishing the invariant. This may look like nitpicking but helps tremendeously when developing code in imperative programming languages.
