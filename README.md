...
Let us take a look at the code shown in the above figure. It first checks if `character` is a letter. In that case, the code would be looking for an identifier. We deal with that case below. If not, the code checks if `character` is a digit. Say, per our example it is `8` as indicated by the green line, so yes. The `if` statement, or rather the `if` condition evaluating to true, takes us to the red start state of our FSM as indicated by a blue line. The following `if` statement checks if `character` is `'0'` or not. If yes, we are done. Computing the numerical value of `'0'` is trivial in this case. It is just `0`. Notice the difference between `'0'` and `0`. The former is the ASCII code of `0` which happens to be the decimal value `48`, not the decimal value `0`. The other important item here is the call to `get_character()` which reads the next character and stores it in `character`, to prepare for subsequent calls to the scanner.

> Invariants!

Thereby, the scanner maintains something very important known as an *invariant*: `character` always contains the character that is to be scanned next. In other words, upon calling the scanner, we assume the invariant holds as a *precondition*. This means that upon calling the scanner for the first time, we actually have to call `get_character()` before that. Try finding that call in the selfie code! Then, the scanner scans characters and, when done, makes sure that the invariant holds again as a *postcondition* to prepare for the next call to the scanner. You may have noticed that the invariant is temporarily violated whenever the scanner began examining `character` until it calls `get_character()` again reestablishing the invariant. This may look like nitpicking but helps tremendeously when develop code.
