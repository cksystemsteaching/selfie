...
Virtual memory enables co-existence of multiple virtual machines on the same physical machine while each virtual machine is *spatially* isolated in physical memory from all other virtual machines. Spatial isolation of virtual machines, and to a lesser degree of processes and threads, enables *concurrency* which allows *time-sharing* physical resources such as a CPU in different order without effect on the outcome. More concurrency means more freedom in scheduling time-shares while keeping the outcome the same.
