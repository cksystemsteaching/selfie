...
How does selfie determine whether something is a value or a pointer?
...
So, all content of the data and stack segments is considered potential roots into the heap segment, and on top of that, all content of the heap segment is also considered potential pointers back into the heap segment. In general, even the content of registers must be considered as roots as well, just not with code generated by the selfie compiler, as the content of the relevant registers is always saved on the stack whenever the garbage collector might run.
...
With the `-nr` option, the `libcstar` garbage collector is configured not to reuse deallocated memory but instead just make deallocated memory, including the involved metadata it uses, unreachable, only to have the garbage collector in `mipster` identify the deallocated memory, including the metadata of the `libcstar` garbage collector, again as unreachable but this time actually make it available for reuse. Without the `-nr` option, the `libcstar` garbage collector keeps all memory ever allocated by the emulated code reachable, even the deallocated memory and all its metadata. In that case, the garbage collector in `mipster` will only find reachable memory and never actually collect any memory, leaving all reuse of memory to the `libcstar` garbage collector. Just remove the `-nr` option from the invocation of selfie to see in the profile data of the output what happens.

What is the purpose of doing that? Well, other than being a cool feature, there is educational value because supporting self-collection forces us to isolate code for accessing address spaces from actual garbage collection code, which is the majority of the overall code, similar to `mipster` and `hypster` sharing most of their virtualization code as well. The getters and setters used by the garbage collector essentially provide the necessary abstraction from accessing different address spaces. See the source code for more details.
