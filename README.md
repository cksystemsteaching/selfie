...
Let us take a look at the code shown in the above figure. It first checks if `character` is a letter. In that case, the code would be looking for an identifier. We deal with that case below. If not, the code checks if `character` is a digit. Say, per our example it is `8` as indicated by the green line, so yes. The `if` statement, or rather the `if` condition evaluating to true, takes us to the red start state of our FSM as indicated by a blue line. The following `if` statement checks if `character` is `'0'` or not. If yes, we are done. This check implements the `|` operator in the EBNF production `integer = "0" | non_zero_digit { digit } .`. Computing the numerical value of `'0'` is trivial in this case. It is just `0`. Notice the difference between `'0'` and `0`. The former is the ASCII code of `0` which happens to be the decimal value `48`, not the decimal value `0`. This is our first and most basic example of the difference of syntax and semantics! The character `'0'` is syntax, the value `0` is semantics, that is, the value `0` is what we mean by the character `'0'`.
...
Then, the scanner scans characters and, when done, needs to make sure that the invariant holds again as a *postcondition* by calling `get_character()` once more before terminating to prepare for the next call to the scanner. Notice that the invariant is temporarily violated whenever the scanner begins examining `character`, making it the currently read character rather than the next character to read, until it calls `get_character()` again, reestablishing the invariant. This may look like nitpicking but reasoning in terms of invariants as well as pre- and postconditions helps tremendeously when developing code in imperative programming languages.

Every time you would like to write down a statement in, say, C\* think of which property of the program state needs to hold before executing the statement so that executing the statement works as intended. This is the precondition. And then think of which property of the program state holds after executing the statement, assuming the precondition is true. This is the postcondition which becomes the precondition for the next statement and so on. Invariants show up in iterative and recursive code, that is, in loops, as we see next, and recursive procedures, which here are our parser procedures that keep calling the scanner, as we see further below. When reasoning about loops and recursive procedures, the key question is what exactly the invariants of those loops and recursive procedures are. An invariant is a property that is both pre- and postcondition of a loop or recursive procedure.
