...
The `-o` option instructs `selfie` to output the generated code into an executable file, here `selfie.m`. Similarly, the `-S` option instructs `selfie` to output the generated code in textual form into an assembly file, here `selfie.s`.
...
We can then check if the code generated by the ARM executable, as output into `selfie1.m` and even `selfie1.s`, is equivalent to the code generated by the RISC-U executable, as output into `selfie2.m` and `selfie2.s`:
