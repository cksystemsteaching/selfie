...
We first look into tracing and then reference counting. The arguably simplest implementation of tracing is a *mark-and-sweep* garbage collector.
...
With the `-nr` option in the invocation of selfie, the `libcstar` garbage collector is instructed not to reuse memory. Instead, it just makes memory that could be reused, including the involved metadata it uses, unreachable even for itself. This enables the garbage collector in `mipster` to identify all that memory, including the metadata of the `libcstar` garbage collector, as unreachable and then make all that memory actually available for reuse. Without the `-nr` option, the `libcstar` garbage collector keeps all memory ever allocated by the emulated code reachable, in particular eventually deallocated memory and all its metadata, to be able to reuse that memory. Thus the garbage collector in `mipster` only finds reachable memory and never actually collects any memory, leaving all reuse of memory to the `libcstar` garbage collector. Just remove the `-nr` option from the invocation of selfie to see in the profile data of the output what happens.
...
The importance of garbage collection has motivated computer scientists to develop ever more advanced versions to improve performance.
