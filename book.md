...
Before taking the next step towards how files are stored and organized, which is surprisingly easy with what we know now, we would like to clarify some important terminology. Computer scientists often call a sequence of characters a *string*, not text, especially if the string is relatively short. One reason is that sequences of characters can be used to encode all kinds of information, not just text. But the more important reason is that strings are stored contiguously in memory and NULL-terminated, that is, with a NULL character at the end. In other words, whenever we are dealing with a string, or an integer, we only need to know the address of where it begins in memory. That's all. Arbitrary text, on the other hand, may be stored contiguously, non-contiguously, or in a combination of the two, as described above, which is similar to how files are stored, as explained next.

### Files

What is a *file*? From the perspective of a computer it is, unsurprisingly, a sequence of bits or in fact a sequence of bytes, any bytes and any number of bytes, even zero bytes, plus one more thing. Can you guess what that is? It is a *name*, which is a string of characters, in our case, ASCII characters which is in the end just another sequence of bits, but for sure stored contiguously and NULL-terminated. That's it! Sure, there are other things like the day and time a file was created and so on but that kind of information is not so important here.
...
The distinction of file and filename is important. Not being aware of that is another typical reason why so many people do not know how to handle computers properly and computer files in particular. Let us try to change that. There is a simple analogy between files and memory. A file is like storage and a filename is like an address. In fact, the choice of possible filenames creates what we call a *namespace*, similar to an address space. The difference is that names are strings and therefore human-readable whereas addresses are pointers which may be 32-bit or even 64-bit unsigned integers that are impossible to remember.

But the more important difference between filenames and addresses is that filenames are *portable*, at least to some extent, whereas addresses are not. In fact, a filename is a portable *abstraction* of a memory address. Computer scientists love abstraction and this is our first example out of many that follow below. A filename allows us to refer to a file without knowing where exactly the file is stored in memory. If the filename is unique among all files lying around in one place the name is actually as good as an address. If there is another file with the same name we need to rename one of the two files to be able to keep them in the same place.

TODO: mention interpretation, directories, hierarchy, indexing, .doc files
