...
Before taking the next step towards how files are stored and organized, which is surprisingly easy with what we know now, we would like to clarify some important terminology. Computer scientists often call a sequence of characters a *string*, not text, especially if the string is relatively short. One reason is that sequences of characters can be used to encode all kinds of information, not just text. But the more important reason is that strings are stored contiguously in memory and NULL-terminated, that is, with a NULL character at the end. In other words, whenever we are dealing with a string, or an integer, in fact, we only need to know the address of where it begins in memory. That's all. Arbitrary text, on the other hand, may be stored contiguously, non-contiguously, or in a combination of the two, as described above, which is similar to how files are stored, as explained next.
