...
The trick is to slow yourself down! So much so that it is almost painful taking such tiny steps when learning something so big. The key issue why computer science, and in fact elementary mathematics, is so elusive to many is because there is a fundamental difference between formal languages such as computer code or mathematical formulae and *natural* languages such as English or German. Formal languages have *formal* syntax and in particular a *formal* semantics. There is no such thing as *casual* code or formulae. In short, everything matters, even the tiniest detail!
...
TODO: describe how else this can be done. Mention that motivation is key not to fall asleep yourself.

The problem with that is that if you miss one step it may be impossible to compensate for that later.
