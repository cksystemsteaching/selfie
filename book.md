...
Before taking the next step towards how files are stored and organized, which is surprisingly easy with what we know now, we would like to clarify some important terminology. Computer scientists call a sequence of characters a *string*, not text, especially if the string is relatively short. The reason is that sequences of characters can be used to encode all kinds of information, not just text. But otherwise, strings are stored in memory just like text. For example, a NULL-terminated string is stored contiguously in memory with a NULL character at the end. In fact, from now on we assume that all strings that we encounter are NULL-terminated, which is the default in most systems today.
