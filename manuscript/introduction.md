...
Why is the self so important? Because resolving the self shows how meaning is created systematically on a machine. Selfie is software written in some programming language. However, selfie defines the meaning of that language which implies that selfie defines its own meaning. This is very similar to an English dictionary written in English. The only difference is that selfie defines meaning formally in such a way that a machine can *understand* it.

The book works by explaining selfie step by step while focusing on the basic principles that are necessary to understand each step. In order to follow the book you will need to download and run [selfie](https://github.com/cksystemsteaching/selfie "selfie"). This requires, yes, a terminal and a C compiler but do not be scared, please. Just think of that terminal as an elderly but wise chatbot and you will be fine. If you have a Mac or a Linux box, terminal and compiler are already there, and easy to get for a Windows machine.