...
![Mipster Self-Compilation](images/mipster-self-compilation.png "Mipster Self-Compilation")
...
What you see is a human-readable version of the machine code in `selfie.m`. The purpose of `selfie.s` is here to study `selfie.m` and eventually understand its semantics. Selfie can even show the assembly code as it is being executed by mipster which helps debugging the code.
...
Let us try that using the `-d` option which invokes mipster in debugging mode. Careful though, there will be a lot of code racing down your screen, omitting most of that here:
...
Let us try and push the envelope a bit further.
...
{line-numbers=off}
```
> ./selfie -c selfie.c -o selfie3.m -m 4 -l selfie3.m -y 2 -c selfie.c -o selfie4.m
  ./selfie: this is selfie's starc compiling selfie.c
  ./selfie: writing code into output file selfie3.m
  ./selfie: this is selfie's mipster executing selfie3.m with 4MB of memory
...
![Hypster Self-Compilation](images/hypster-self-compilation.png "Hypster Self-Compilation")