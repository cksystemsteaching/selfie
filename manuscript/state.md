...
Here is `countdown.s` but only showing the instructions that will actually be executed when running the program. The instructions that will not be executed, shown as `...`, are generated by starc because they may be needed by some programs, just not the countdown program. The code will be loaded in chunks of 32-bit words into memory starting at address `0x0`, as indicated by `0x0(~1)` in the first line.  The instruction at this address, for example, is encoded in the word `0x240801EC` which stands for `addiu $t0,$zero,492`. The number `~1` in parentheses is the approximate line number in `countdown.c` for which starc generated an instruction. Even though there is a comment at Line 1 in `countdown.c` this still makes sense because starc always generates some instructions before compiling any actual source code. Try to find the four instructions that starc generated for `bar = bar - 1;` in Line 13 of `countdown.c`, for example! They, along with the others, are explained below.

{line-numbers=off}
```
0x0(~1): 0x240801EC: addiu $t0,$zero,492
...
...
0x17C(~11): 0x27BDFFFC: addiu $sp,$sp,-4
0x180(~11): 0xAFBF0000: sw $ra,0($sp)
0x184(~11): 0x27BDFFFC: addiu $sp,$sp,-4
0x188(~11): 0xAFBE0000: sw $fp,0($sp)
0x18C(~11): 0x27BE0000: addiu $fp,$sp,0
0x190(~11): 0x8F88FFFC: lw $t0,-4($gp)
0x194(~11): 0x24090000: addiu $t1,$zero,0
0x198(~11): 0x0128402A: slt $t0,$t1,$t0
0x19C(~11): 0x10080007: beq $zero,$t0,7[0x1BC]
0x1A0(~11): 0x00000000: nop
0x1A4(~13): 0x8F88FFFC: lw $t0,-4($gp)
0x1A8(~13): 0x24090001: addiu $t1,$zero,1
0x1AC(~13): 0x01094023: subu $t0,$t0,$t1
0x1B0(~13): 0xAF88FFFC: sw $t0,-4($gp)
0x1B4(~19): 0x1000FFF6: beq $zero,$zero,-10[0x190]
0x1B8(~19): 0x00000000: nop
0x1BC(~19): 0x8F88FFFC: lw $t0,-4($gp)
0x1C0(~19): 0x00081021: addu $v0,$zero,$t0
0x1C4(~19): 0x08000073: j 0x73[0x1CC]
0x1C8(~19): 0x00000000: nop
0x1CC(~20): 0x27DD0000: addiu $sp,$fp,0
0x1D0(~20): 0x8FBE0000: lw $fp,0($sp)
0x1D4(~20): 0x27BD0004: addiu $sp,$sp,4
0x1D8(~20): 0x8FBF0000: lw $ra,0($sp)
0x1DC(~20): 0x27BD0004: addiu $sp,$sp,4
0x1E0(~20): 0x03E00008: jr $ra
0x1E4(~20): 0x00000000: nop
```
...
Line 9 shows the first instruction executed by mipster and reads as follows. The PC denoted by `$pc` points to memory address `0x0`. The instruction at this address is encoded in the word `0x240801EC` which stands for `addiu $t0,$zero,492`, as mentioned before.

#### [addiu](http://github.com/cksystemsteaching/selfie/blob/5c0fed59da834b8cce6077283c50f2054b409679/selfie.c#L5698-L5736)

Now, here is the interesting part. The remaining output `$t0=0,$zero=0 -> $t0=492` tells us which part of the state space (memory, registers) the instruction depends on and what the affected state actually is right before executing the instruction (`$t0=0,$zero=0` to the left of the arrow `->`) and which part of the state space changes and how after executing the instruction (`$t0=492` to the right of `->`). In other words, the instruction depends on the values in the two registers `$t0` and `$zero` that both contain 0 and the instruction changes the value in register `$t0` to 492. This is because `addiu $t0,$zero,492` instructs the processor to *add* the value 492 to the value in register `$zero` which is always 0 and store the result in register `$t0`.
...
Now, the issue is that representing 0x3FFFFFC in binary requires 26 bits but the signed integer argument of `addiu` is only 16 bits. The solution is to [break that large number into smaller numbers using left and logical right shift and then arithmetics to recompute the original number](http://github.com/cksystemsteaching/selfie/blob/506beef2383cf656ce40ff5fac99d2890b0d9fc6/selfie.c#L2617-L2657). Here, we exploit that with signed 32-bit integer arithmetics 2^26^-4=((2^26^-4)/2^14^)\*2^14^+((2^26^-4)-((2^26^-4)/2^14^)\*2^14^)=4095\*16384+16380 where all three numbers fit into 16-bit signed integers and can therefore be loaded into registers through immediate addressing with `addiu`.
