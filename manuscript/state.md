...
## [While Statement](https://github.com/cksystemsteaching/selfie/blob/bf66b7935afa629dbf1bfc7f3d102cdae8a4f23a/selfie.c#L3873-L3939)
...
Before explaining the instructions that implement the body of the while loop we focus on the last instruction that implements the while statement. It is the [`jal $zero,-8[0x10148]`](https://github.com/cksystemsteaching/selfie/blob/bf66b7935afa629dbf1bfc7f3d102cdae8a4f23a/selfie.c#L3929) instruction at address `0x10168` which is in fact used here for *backward jumping*. The instruction unconditionally *jumps backwards* by 8 instructions (due to the negative offset `-8`) to the first instruction that implements the while statement at address `0x10148`. In other words, the unconditional backward jump completes the loop.
...
## [Return Statement](https://github.com/cksystemsteaching/selfie/blob/c862937fa12ff314b7ac4410224950623cf4f13d/selfie.c#L4033-L4066)
...
#### [add](https://github.com/cksystemsteaching/selfie/blob/a6959c4ecace550495c600531d489d9fe55c57d0/selfie.c#L6643-L6651)
...
#### [mul](https://github.com/cksystemsteaching/selfie/blob/a6959c4ecace550495c600531d489d9fe55c57d0/selfie.c#L6643-L6651)
...
#### [divu](https://github.com/cksystemsteaching/selfie/blob/a6959c4ecace550495c600531d489d9fe55c57d0/selfie.c#L6898-L6911)
...
#### [remu](https://github.com/cksystemsteaching/selfie/blob/a6959c4ecace550495c600531d489d9fe55c57d0/selfie.c#L6972-L6985)

The `remu` instruction is just like the `divu` instruction except that it computes the remainder.

## Pointer

Before concluding this chapter we go back to the ["Hello World!" program](https://github.com/cksystemsteaching/selfie/blob/06498c53d5fa503d0eabc25161f38feccd19135c/manuscript/code/hello-world.c) presented in the previous chapter:
...
The next interesting instruction is the [`ld $t0,0($t0)`](https://github.com/cksystemsteaching/selfie/blob/06498c53d5fa503d0eabc25161f38feccd19135c/selfie.c#L3664) instruction at address `0x10158` which starc generated for the occurrence of the dereference operator in `while (*foo != 0)`. Before that instruction, the [`ld $t0,-16($gp)`](https://github.com/cksystemsteaching/selfie/blob/06498c53d5fa503d0eabc25161f38feccd19135c/selfie.c#L3283) instruction at address `0x10154` loads the value of `foo` into `$t0`. The [`ld $t0,0($t0)`](https://github.com/cksystemsteaching/selfie/blob/06498c53d5fa503d0eabc25161f38feccd19135c/selfie.c#L3664) instruction *dereferences* the pointer `foo` by loading the word into `$t0` from memory where `$t0` points to. After executing the instruction, `$t0` thus contains the value that encodes `"Hello Wo"`. Really? Check the mipster output `$t0=0x101D0,mem[0x101D0]=8022916924116329800 |- $t0=66000(0x101D0) -> $t0=8022916924116329800(0x6F57206F6C6C6548)=mem[0x101D0]` for that instruction. The decimal number `8022916924116329800` loaded into `$t0` is equal to `0x6F57206F6C6C6548` in hexadecimal notation. The hexadecimal number `0x48` is the ASCII code for the uppercase letter `H`. Moreover, the hexadecimal numbers `0x65` and `0x6C` are the ASCII codes for the lowercase letters `e` and `l`, respectively. The rest is straightforward. Very nice!

Before concluding this chapter, let us have a quick look at the code generated for the assignment `foo = foo + 1` in Line 21. With the values of `foo` and `1` loaded into `$t0` and `$t1`, respectively, the [`addi $t2,$zero,8`](https://github.com/cksystemsteaching/selfie/blob/06498c53d5fa503d0eabc25161f38feccd19135c/selfie.c#L4588) instruction at address `0x101A0` loads 8, the size of an integer value in bytes, into register `$t2` which is another register for temporary results. It is [register 7](https://github.com/cksystemsteaching/selfie/blob/06498c53d5fa503d0eabc25161f38feccd19135c/selfie.c#L656) among the 32 general-purpose registers.

The following [`mul $t1,$t1,$t2`](https://github.com/cksystemsteaching/selfie/blob/06498c53d5fa503d0eabc25161f38feccd19135c/selfie.c#L4589) instructions computes the increment 8 by multiplying the values of `$t1` and `$t2` and store the result in `$t1`. The following [`add $t0,$t0,$t1`](https://github.com/cksystemsteaching/selfie/blob/06498c53d5fa503d0eabc25161f38feccd19135c/selfie.c#L3764) instruction increments the value of `$t0` by 8. The final [`sd $t0,-16($gp)`](https://github.com/cksystemsteaching/selfie/blob/06498c53d5fa503d0eabc25161f38feccd19135c/selfie.c#L4202) instruction performs the actual assignment of the value of `foo + 1` according to pointer arithmetics stored in `$t0` into the word that stores the value of `foo`. That's all.
