...
The purpose of the first sixteen instructions executed by mipster is to initialize the machine and get it ready for running the code implementing the `main` procedure. Initially, the PC denoted by `$pc` points to memory address `0x0`. The instruction at this address is thus the first instruction that will be executed by the machine. The instruction is encoded in the word `0x240801EC` which stands for `addiu $t0,$zero,492`, as mentioned before.
