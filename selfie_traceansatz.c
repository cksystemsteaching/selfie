...
void symbolic_prepare_regs(uint64_t context);

uint64_t vtc(uint64_t counter);
...
uint64_t lower = 0; // bounds of symbolic trace
uint64_t upper = 1; // bounds of symbolic trace
...
void initInterpreter() {
  ...
  *(EXCEPTIONS + EXCEPTION_UNKNOWNINSTRUCTION) = (uint64_t) "unknown instruction";
  ...
  if (symbolic)
    values = zalloc(maxTraceLength * SIZEOFUINT64 * 2);
  else
...
void implementExit(uint64_t* context) {
  if (symbolic)
    setExitCode(context, *(values + vtc(*(getRegs(context) + REG_A0)) + lower));
  else
...
void symbolic_prepare_regs(uint64_t context) {
  ...
  while (reg < NUMBEROFREGISTERS) {
    if (isSystemRegister(reg)) {
      *(values + vtc(tc) + lower) = *(getRegs(context) + reg);
      *(values + vtc(tc) + upper) = *(getRegs(context) + reg);
...
// void updateRegState(uint64_t value) {
//   *(values + (tc % maxTraceLength)) = value;
//
//   tc = tc + 1;
// }
...
void symbolic_do_lui() {
  ...
  if (rd != REG_ZR) {
    // semantics of lui
    *(values + vtc(tc) + lower) = leftShift(imm, 12);
    *(values + vtc(tc) + upper) = leftShift(imm, 12);
  }
...
void symbolic_do_addi() {
  ...
  if (rd != REG_ZR) {
    // semantics of addi
    *(values + vtc(tc) + lower) = *(values + vtc(*(registers + rs1)) + lower) + imm;
    *(values + vtc(tc) + upper) = *(values + vtc(*(registers + rs1)) + upper) + imm;
  }
...
void symbolic_do_add() {
  if (rd != REG_ZR) {
    // semantics of add
    *(values + vtc(tc) + lower) = *(values + vtc(*(registers + rs1)) + lower) + *(values + vtc(*(registers + rs2)) + lower);
    *(values + vtc(tc) + upper) = *(values + vtc(*(registers + rs1)) + upper) + *(values + vtc(*(registers + rs2)) + upper);
  }
...
void symbolic_do_sub() {
  if (rd != REG_ZR) {
    // semantics of sub
    *(values + vtc(tc) + lower) = *(values + vtc(*(registers + rs1)) + lower) - *(values + vtc(*(registers + rs2)) + lower);
    *(values + vtc(tc) + upper) = *(values + vtc(*(registers + rs1)) + upper) - *(values + vtc(*(registers + rs1)) + upper);
  }
...
void symbolic_do_divu() {
  ...
  if (*(values + *(registers + rs2) + lower) == *(values + *(registers + rs2) + upper)) {
    if (*(values + *(registers + rs2) + lower) != 0) {
      if (rd != REG_ZR) {
        // semantics of divu
        *(values + vtc(tc) + lower) = *(values + vtc(*(registers + rs1)) + lower) / *(values + vtc(*(registers + rs2)) + lower);
        *(values + vtc(tc) + upper) = *(values + vtc(*(registers + rs1)) + upper) / *(values + vtc(*(registers + rs2)) + upper);
...
void symbolic_do_sltu() {
  ...
  if (*(values + vtc(*(registers + rs1)) + lower) != *(values + vtc(*(registers + rs2)) + upper))
    if (*(values + vtc(*(registers + rs1)) + lower) != *(values + vtc(*(registers + rs2)) + upper))
      invalid = 1;
  ...
  if (invalid == 0) {
    if (rd != REG_ZR) {
      ...
      if (*(values + vtc(*(registers + rs1)) + upper) < *(values + vtc(*(registers + rs2)) + lower)) {
        *(values + vtc(tc) + lower) = 1;
        *(values + vtc(tc) + upper) = 1;
      } else {
        *(values + vtc(tc) + lower) = 0;
        *(values + vtc(tc) + upper) = 0;
      }
    }
  }
...
void print_ld_before() {
  ...
  if (symbolic)
    vaddr = *(values + vtc(*(registers + rs1)) + lower) + imm;
  else
  ...
  if (isValidVirtualAddress(vaddr))
    if (isVirtualAddressMapped(pt, vaddr)) {
      ...
      if (isSystemRegister(rd)) {
        if (symbolic) {
          printHexadecimal(*(values + vtc(loadVirtualMemory(pt, vaddr)) + lower), 0);
          print((uint64_t*) ",");
          printHexadecimal(*(values + vtc(loadVirtualMemory(pt, vaddr)) + upper), 0);
        } else
...
uint64_t symbolic_do_ld() {
  ...
  if (isValidVirtualAddress(vaddr)) {
    if (isVirtualAddressMapped(pt, vaddr)) {
      if (rd != REG_ZR) {
        // semantics of ld
        *(values + vtc(tc) + lower) = *(values + vtc(loadVirtualMemory(pt, vaddr)) + lower);
        *(values + vtc(tc) + upper) = *(values + vtc(loadVirtualMemory(pt, vaddr)) + upper);
      }
...
uint64_t symbolic_do_sd() {
  ...
  if (isValidVirtualAddress(vaddr)) {
    if (isVirtualAddressMapped(pt, vaddr)) {
      // semantics of sd
      *(values + vtc(tc) + lower) = *(values + vtc(*(registers + rs2)) + lower);
      *(values + vtc(tc) + upper) = *(values + vtc(*(registers + rs2)) + upper);
...
void symbolic_do_beq() {
  ...
  // semantics of beq
  if (*(values + vtc(*(registers + rs1)) + lower) == *(values + vtc(*(registers + rs2)) + lower))
    pc = pc + imm;
  else
    pc = pc + INSTRUCTIONSIZE;
...
void symbolic_do_jal() {
  ...
  if (rd != REG_ZR) {
    // first link
    *(values + vtc(tc) + lower) = pc + INSTRUCTIONSIZE;
    *(values + vtc(tc) + upper) = pc + INSTRUCTIONSIZE;
...
void symbolic_do_jalr() {
  ...
  if (rd == REG_ZR)
    ...
    pc = leftShift(rightShift(*(values + vtc(*(registers + rs1)) + lower) + imm, 1), 1);
  else {
    // slow path: first prepare jump, then link, just in case rd == rs1

    // prepare jump with LSB reset
    next_pc = leftShift(rightShift(*(values + vtc(*(registers + rs1)) + lower) + imm, 1), 1);

    // link to next instruction
    *(values + tc + lower) = pc + INSTRUCTIONSIZE;
    *(values + tc + upper) = pc + INSTRUCTIONSIZE;
...
void symbolic_do_ecall() {
  ...
  if (*(values + vtc(*(registers + REG_A7)) + lower) == SYSCALL_SWITCH)
    implementSwitch();
  else
...
void printRegisterHexadecimal(uint64_t r) {
  ...
  if (symbolic) {
    print((uint64_t*) "[");
    printHexadecimal(*(values + vtc(*(registers + r)) + lower), 0);
    print((uint64_t*) ",");
    printHexadecimal(*(values + vtc(*(registers + r)) + upper), 0);
    print((uint64_t*) "]");
...
void printRegisterValue(uint64_t r) {
  if (isSystemRegister(r))
    ...
    if (symbolic) {
      print((uint64_t*) "[");
      printInteger(*(values + vtc(*(registers + r)) + lower));
      print((uint64_t*) ",");
      printInteger(*(values + vtc(*(registers + r)) + upper));
      print((uint64_t*) "]");
    } else {
...
uint64_t handleSystemCalls(uint64_t* context) {
  ...
  if (getException(context) == EXCEPTION_SYSCALL) {
    ...
    if (symbolic)
      a7 = *(values + vtc(*(getRegs(context) + REG_A7)) + lower);
    else
...
uint64_t vipster(uint64_t* toContext) {
  ...
  symbolic_prepare_regs(toContext);
