...
def test_interleaved_output(command, interleaved_msg, number_of_interleaved, msg):
    test_execution(command, msg, success_criteria=lambda code, out: is_interleaved_output(
        code, out, interleaved_msg, number_of_interleaved))
...
def grade():
    ...
    for stage in range(0, len(number_of_positive_tests_passed)):
        ...
        number_of_tests = number_of_positive_tests_passed[stage] + \
            number_of_positive_tests_failed[stage]
        number_of_tests += number_of_negative_tests_passed[stage] + \
            number_of_negative_tests_failed[stage]

        number_of_tests_passed = number_of_positive_tests_passed[stage] + \
            number_of_negative_tests_passed[stage]
    ...
    number_of_tests_passed = sum(
        number_of_positive_tests_passed) + sum(number_of_negative_tests_passed)

    number_of_tests = sum(number_of_positive_tests_passed) + \
        sum(number_of_positive_tests_failed)
    number_of_tests += sum(number_of_negative_tests_passed) + \
        sum(number_of_negative_tests_failed)
...
def print_usage():
    ...
    width = max(
        map(lambda x: 0 if x[2] is None else len(x[2]), defined_options))
        ...
        if args[i] in options:
            ...
            if defined_options[index][2] is None:
                ...
                if len(args) > i:
                    ...
                    print('option flag "' + defined_options[index][0] +
                          '" needs an argument ' + defined_options[index][2])
                    exit(1)
        else:
